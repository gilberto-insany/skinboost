...
test("minimal and explore have different, explicit demonstrative contents", () => {
  const minimal = buildRoutine({
    approach: "Poucos passos",
    budget: "Até R$ 150",
  });
  const explore = buildRoutine({
    approach: "Quero explorar possibilidades",
    budget: "Até R$ 250",
  });
  ...
  assert.match(
    explore.limitations.join(" "),
    /preços fictícios.*economia são simuladas/,
  );
  assert.ok(
    explore.products.every(
      (product) => product.reason && product.source.status === "concept",
    ),
  );
});
...
test("budget reduces optional steps and never silently exceeds the cap", () => {
  for (const budget of [
    0, 30, 48.99, 49, 69, 100, 117.99, 118, 150, 196.99, 197,
  ]) {
    const result = buildRoutine({ approach: "Explorar", budget });
    assert.ok(
      result.subtotal <= budget,
      `budget ${budget}, total ${result.subtotal}`,
    );
    assert.equal(result.withinBudget, true);
    assert.equal(
      result.subtotal,
      result.products.reduce((sum, item) => sum + item.price, 0),
    );
    if (result.products.length < 3)
      assert.match(result.limitations.join(" "), /orçamento reduziu/);
  }
  assert.deepEqual(ids(buildRoutine({ approach: "Explorar", budget: 150 })), [
    "cleanse",
    "comfort",
  ]);
...
test("known products are preserved by omitting duplicate categories", () => {
  const result = buildRoutine({
    approach: "Explorar",
    existing: "Já uso sabonete e hidratante",
    budget: 100,
  });
  ...
  assert.deepEqual(
    ids(buildRoutine({ existing: ["Cleanse", "Comfort"], budget: 0 })),
    [],
  );
  assert.match(
    buildRoutine({ existing: "Cleanse e Comfort" }).explanation,
    /Nenhum item repetido/,
  );
  assert.deepEqual(
    ids(
      buildRoutine({
        approach: "Ajustar o que já uso",
  ...
  assert.deepEqual(
    ids(
      buildRoutine({
        approach: "Ajustar o que já uso",
        existing: "Já uso limpeza e hidratação",
        budget: 150,
      }),
    ),
    [],
  );
  assert.deepEqual(ids(buildRoutine({ existing: "Nenhum produto" })), [
    "cleanse",
    "comfort",
  ]);
});
...
test("negative existing-product statements do not count as products in use", () => {
  assert.deepEqual(
    ids(buildRoutine({ existing: "Não uso sabonete. Uso hidratante." })),
    ["cleanse"],
  );
  assert.deepEqual(
    ids(buildRoutine({ existing: "Sem limpador, uso hidratante sem perfume" })),
    ["cleanse"],
  );
  assert.deepEqual(ids(buildRoutine({ existing: "Nenhum" })), [
    "cleanse",
    "comfort",
  ]);
  assert.deepEqual(ids(buildRoutine({ existing: "Uso protetor solar" })), [
    "cleanse",
    "comfort",
  ]);
});
...
test("hydration priority cannot be replaced with an unrelated cheaper add-on", () => {
  assert.deepEqual(ids(buildRoutine({ goal: "Hidratação", budget: 70 })), [
    "comfort",
  ]);
  assert.deepEqual(ids(buildRoutine({ goal: "Hidratação", budget: 60 })), []);
  assert.match(
    buildRoutine({ goal: "Hidratação", budget: 60 }).explanation,
    /não comporta o primeiro item/,
  );
});

test("display order follows the routine after hydration budget selection", () => {
  const full = buildRoutine({
    goal: "hidratação",
    approach: "Explorar possibilidades",
    budget: 250,
  });
...
test("sensitivity remains a limitation and never produces clinical-suitability claims", () => {
  const result = buildRoutine({
    sensitivity: "Tenho sensibilidade ou restrições",
    budget: 150,
  });
  assert.equal(result.products.length, 2);
  assert.match(
    result.limitations.join(" "),
    /não permite concluir tolerância ou adequação/,
  );
  assert.ok(
    result.products.every(
      (product) =>
        !/ideal para pele sensível|hipoalergênico|clinicamente testado/i.test(
          product.reason,
        ),
    ),
  );
...
test("declared absence of known restrictions does not become a sensitivity claim", () => {
  ...
  assert.ok(
    !result.limitations.some((text) => /sensibilidade informada/.test(text)),
  );
  assert.match(
    result.limitations.join(" "),
    /não avalia adequação, segurança ou eficácia/,
  );
...
test("goals explain the declared preference without inventing efficacy", () => {
  ...
  assert.ok(
    oily.products.every((product) =>
      /interesse em oleosidade/.test(product.reason),
    ),
  );
  assert.ok(
    oily.products.every((product) =>
      /não comprova controle de oleosidade/.test(product.reason),
    ),
  );
  assert.match(
    buildRoutine({ goal: "conhecer" }).products[0].reason,
    /conhecer as opções/,
  );
  assert.match(
    buildRoutine({ goal: "hidratação" }).products.find(
      (product) => product.id === "comfort",
    ).reason,
    /ganha prioridade/,
  );
});
...
test("medical routing catches explicit diagnosis, disease treatment and medication asks", () => {
  for (const text of [
    "Diagnostique minha pele",
    "Qual é o diagnóstico?",
    "Quero tratamento para rosácea",
    "Como tratar acne?",
    "Essa pinta pode ser melanoma?",
    "Me diga se tenho dermatite",
    "Qual medicamento devo usar?",
    "Prescreva um remédio",
  ]) {
    ...
    assert.match(
      result.explanation,
      /não responde a pedidos de diagnóstico ou tratamento/,
    );
  }
});
...
test("ordinary skincare goals and explicit diagnostic disclaimers are not medical asks", () => {
  for (const text of [
    "Tenho pele seca",
    "Quero uma rotina para pele oleosa",
    "Minha pele é sensível",
    "Quero hidratação",
    "Tenho acne e quero uma rotina simples",
    "Sem diagnóstico, só uma rotina simples",
    "Não quero diagnóstico",
    "Não quero tratar acne, só entender os produtos",
    "Uso um medicamento e hidratante",
    "Fui diagnosticada com acne e quero entender uma rotina simples",
    "Meu médico prescreveu um medicamento",
  ]) {
...
test("photo validation is optional and validates metadata, not photo quality", () => {
  ...
  assert.equal(
    validatePhoto({ type: "image/jpeg", size: 1024, name: "foto.jpg" }),
    "",
  );
  assert.equal(
    validatePhoto({
  ...
  assert.equal(
    validatePhoto({ type: "image/webp", size: 50, name: "foto.webp" }),
    "",
  );
  for (const file of [
    { type: "image/svg+xml", size: 100 },
    { type: "image/jpeg", size: 0 },
    { type: "image/jpeg", size: Infinity },
    { type: "image/jpeg", size: 10 * 1024 * 1024 + 1 },
    { type: "image/png", size: 10, name: "arquivo.svg" },
  ])
    assert.ok(validatePhoto(file));
});

test("calls preserve input, defaults and the catalog across local edits", () => {
  const context = {
    intent: "Poucos passos",
    existing: "Uso sabonete",
    budget: 150,
  };
  ...
  assert.match(
    CATALOG.find((product) => product.id === "comfort").source.title,
    /SkinBoost/,
  );
