...
import {
  ...
  composerQuestions as questions,
} from "./chat/conversation.js";
export { createExperienceState };
...
export function mountExperience(element, options = {}) {
  ...
  const log = (name, detail = {}) => {
    state.events.push({ name, ...detail });
    options.onChange?.(state);
  };
  const seed = () => {
    state.context = {
      ...state.context,
      intent: "Quero uma rotina simples, aproveitando o que já uso.",
      goal: "hidratação",
    ...
    state.routine = buildRoutine(state.context);
    state.selected = state.routine.products.map((p) => p.id);
  };
  state.step = options.initialStep || state.step;
  if (
    ["routine", "cart", "checkout", "checkin"].includes(state.step) &&
    !state.routine
  )
    seed();
  if (state.step === "context" && !state.context.intent)
    state.context.intent = "Quero começar com uma rotina simples.";
  element.classList.add("sb-experience");

  function go(step, extra = {}) {
    Object.assign(state, transitionExperience(state, step, extra));
    render();
  }
  function say(text) {
    const node = local("[data-live]");
    if (node) node.textContent = text;
  }
  function notify(text) {
    say(text);
    clearTimeout(noticeTimer);
    noticeTimer = setTimeout(() => say(""), 4500);
  }
  function button(text, action, variant = "primary", attrs = "") {
    return `<button type="button" class="sx-button sx-${variant}" data-action="${action}" ${attrs}>${text}</button>`;
  }
  function title(kicker, heading, description = "") {
    return `<div class="sx-heading"><span class="sx-kicker">${kicker}</span><h2 id="dialog-title" tabindex="-1">${heading}</h2>${description ? `<p>${description}</p>` : ""}</div>`;
  }
  function hint(text, type = "info") {
    return `<div class="sx-hint sx-${type}">${icon(type === "alert" ? "info" : "leaf")}<p>${text}</p></div>`;
  }
  function priceNote() {
    return '<p class="sx-footnote">Valores fictícios · demonstração de escolha e cálculo, sem oferta comercial.</p>';
  }
  ...
  function rail() {
    const phase = [
      ...
      ? 0
      : ["routine", "source", "comparison", "checkin"].includes(state.step)
        ? 1
        : 2;
  ...
  function render() {
    ...
    requestAnimationFrame(() => {
      if (!disposed) {
        local("#dialog-title")?.focus({ preventScroll: true });
        element.closest("dialog")?.scrollTo(0, 0);
        element.closest(".composer-shell")?.scrollTo(0, 0);
      }
    });
  }
  function welcome() {
  ...
  function photoChip() {
    return state.photoName
      ? `<div class="sx-file sx-composer-file">${icon("file-image")}<span>${esc(state.photoName)}</span>${button(icon("x") + '<span class="sr-only">Remover foto</span>', "remove-photo", "icon")}</div><p class="sx-footnote">Arquivo só nesta sessão. Sem envio ou análise.</p>`
      : "";
  }
  function attachment() {
    return `<label class="sx-attach">${icon("camera")} ${state.photoName ? "Trocar foto" : "Adicionar foto"}<span>opcional</span><input class="sr-only" type="file" data-photo accept="image/jpeg,image/png,image/webp"></label>`;
  }
  function conversationTrail() {
    return `<details class="sx-conversation"><summary>${icon("chat-circle-text")} Seu pedido e suas respostas ${icon("caret-down")}</summary><div class="sx-user-message"><span>VOCÊ</span>${esc(state.context.intent)}</div>${questions
      .filter((q) => state.context[q.key])
      .map(
        (q) =>
          `<div class="sx-answer-row"><span>${esc(q.title)}</span><strong>${esc(state.context[q.key])}</strong>${button("Editar", "edit-field", "text", `data-key="${q.key}"`)}</div>`,
      )
      .join("")}</details>`;
  }
  ...
  function processing() {
    return `${title("PREPARANDO SEU EXEMPLO", "Transformando contexto<br>em escolhas.", "Comparando as preferências informadas com as regras locais do catálogo demonstrativo.")}<div class="sx-processing">${icon("circles-three")}<span>Organizando a apresentação da rotina…</span></div>${button("Cancelar e manter respostas", "cancel", "secondary")}`;
  }
  function paused() {
    return `${title("VOCÊ ESTÁ NO CONTROLE", "Suas respostas<br>continuam aqui.", "O preparo foi interrompido. Retome quando quiser ou ajuste seu contexto.")}<div class="sx-actions">${button("Rever contexto", "review", "secondary")}${button("Retomar", "generate")}</div>`;
  }
  function scope() {
    return `${title("UM LIMITE IMPORTANTE", "Essa dúvida merece<br>um profissional.", "Este exemplo organiza uma rotina cosmética. Não identifica doenças em fotos nem indica tratamento para sintomas.")}<div class="sx-user-message">${esc(state.context.intent)}</div>${hint("Podemos continuar explorando as funções dos produtos, sem transformar o seu relato em diagnóstico.")}<div class="sx-actions">${button("Reformular meu pedido", "edit-intent", "secondary")}${button("Conhecer o básico", "basic")}</div>`;
  }
  ...
  function routine() {
    ...
    if (!r)
      return `${title("SUA ROTINA", "Vamos começar<br>pelo contexto?", "Suas preferências ajudam a construir um exemplo com sentido.")}${button("Começar conversa", "welcome")}`;
  ...
  function costBars(r) {
    ...
    return `<div class="sx-bars" role="img" aria-label="Comparação ilustrativa. Referência ${money(r.comparisonTotal)}. Sua seleção ${money(r.subtotal)}.">${[
      ["Referência fictícia", r.comparisonTotal],
      ["Sua seleção fictícia", r.subtotal],
    ]
      .map(
        ([label, value]) =>
          `<div><div class="sx-bar-label"><span>${label}</span><strong>${money(value)}</strong></div><div class="sx-bar-track"><span style="width:${(value / max) * 100}%"></span></div></div>`,
      )
      .join("")}</div>`;
  }
  function comparison() {
  ...
  function generate() {
    ...
    timer = setTimeout(() => {
      ...
      try {
        state.routine = buildRoutine(state.context);
        state.selected = state.routine.products.map((p) => p.id);
        state.orderReviewed = false;
        state.returnStep = "";
        log("routine_ready", { count: state.selected.length });
        go("routine");
      } catch {
        state.step = "paused";
        state.error =
          "Não foi possível organizar o exemplo. Suas respostas estão preservadas. Tente retomar.";
        render();
      }
    }, 350);
  }
  function editField(key) {
    ...
    if (i < 0) return;
    state.returnStep = state.routine ? "routine" : "review";
    go("context", { question: i });
  }
  function copyRoutine() {
    ...
    if (!navigator.clipboard) {
      notify(
        "Não foi possível copiar neste navegador. Selecione o texto da rotina.",
      );
      return;
    }
    navigator.clipboard
      .writeText(text)
      .then(() =>
        notify("Rotina copiada. Os limites da demonstração foram incluídos."),
      )
      .catch(() =>
        notify(
          "Não foi possível copiar. Selecione e copie as informações da rotina.",
        ),
      );
  }
  function actionHandler(e) {
    ...
    switch (action) {
      ...
      case "welcome":
        ...
        break;
      case "preset":
        state.context.intent = target.dataset.value;
        render();
        local("#sx-intent")?.focus();
        break;
      case "demo":
        seed();
        state.revision = 0;
      ...
      case "edit-all":
        ...
        break;
      case "edit-field":
        editField(target.dataset.key);
        break;
      case "edit-intent":
        go("welcome");
        break;
      case "back-question":
        if (state.returnStep) go(state.returnStep);
        else if (state.question > 0)
          go("context", { question: state.question - 1 });
        else go("welcome");
        break;
      case "generate":
        generate();
      ...
      case "cancel":
        clearTimeout(timer);
        go("paused");
        log("routine_cancelled");
        break;
      case "basic":
        state.context.intent =
          "Quero conhecer as funções básicas de uma rotina cosmética.";
        go("context", { question: 0 });
        break;
      case "source":
        state.sourceId = target.dataset.product;
        go("source");
        log("source_opened", { productId: state.sourceId });
        break;
      case "adjust-few":
      ...
      case "checkout":
        if (state.selected.length) {
          go("checkout");
          log("checkout_demo_opened", { count: state.selected.length });
        }
        break;
      case "finish":
        state.orderReviewed = true;
        go("routine");
        notify(
          "Sua seleção está guardada nesta sessão. Nenhuma compra foi realizada.",
        );
        log("selection_saved");
        break;
      case "copy":
      ...
      case "checkin":
        if (!state.routine) {
          go("welcome");
          notify("Comece por uma rotina para fazer o check-in.");
        } else go("checkin");
        break;
      case "remove-photo":
        state.error = "";
        ...
        render();
        notify("Foto removida. Suas respostas foram mantidas.");
        break;
      case "privacy":
        ...
        break;
      case "privacy-back":
        go(state.previousStep || "welcome");
        break;
      case "reset":
        Object.assign(state, createExperienceState());
        options.onPhotoChange?.("");
        options.onReset?.();
        render();
        notify("Respostas, foto e registros apagados desta sessão.");
        break;
      case "review":
      case "routine":
        go(action);
  ...
  function submitHandler(e) {
    ...
    if (form.dataset.form === "intent") {
      const text = String(data.get("intent") || "").trim();
      if (!text) {
        state.error =
          "Conte o que gostaria de entender ou escolha um ponto de partida.";
        render();
        return;
      }
      state.context.intent = text;
      log("conversation_started");
      if (isMedicalRequest(text)) {
        go("scope");
        return;
      }
      const missing = questions.findIndex((q) => !state.context[q.key]);
      go(missing < 0 ? "review" : "context", {
        question: Math.max(0, missing),
      });
    }
    if (form.dataset.form === "question") {
      const q = questions[state.question],
        value = String(data.get("custom") || data.get("answer") || "").trim();
      if (!value) {
        state.error =
          "Escolha uma opção" + (q.custom ? " ou escreva sua resposta." : ".");
        render();
        return;
      }
      if (
        q.key === "budget" &&
        !q.choices.some((c) => c[0] === value) &&
        !/^\s*(R\$\s*)?\d+([.,]\d{1,2})?\s*$/.test(value)
      ) {
        state.error =
          "Informe um valor em reais, como 90 ou R$ 90,50, ou escolha uma opção.";
        render();
        return;
      }
      ...
      if (state.returnStep) {
        state.revision++;
        go("review");
      } else if (state.question < questions.length - 1)
        go("context", { question: state.question + 1 });
      else go("review");
    }
    if (form.dataset.form === "checkin") {
      ...
      if (!status) {
        state.error =
          "Escolha como foi sua experiência para guardar o check-in.";
        render();
        return;
      }
      state.checkin = { status, note: String(data.get("note") || "").trim() };
      log("checkin_saved");
      if (status === "Preciso simplificar") {
        state.context.approach = "Poucos passos";
        state.revision++;
        go("review");
      } else if (status === "Quero rever o custo") editField("budget");
      else {
        go("routine");
        notify("Check-in guardado nesta sessão.");
      }
    }
  }
  function changeHandler(e) {
    if (e.target.matches("[data-photo]")) {
      ...
      render();
      notify(
        "Foto anexada. Nesta demonstração, a imagem não é enviada ou analisada.",
      );
    }
    if (e.target.matches('input[name="answer"]')) {
      if (local("#sx-custom")) local("#sx-custom").value = "";
      all(".sx-option").forEach((el) =>
        el.classList.toggle("selected", el.querySelector("input")?.checked),
      );
    }
    if (e.target.matches("[data-cart-item]")) {
      state.selected = all("[data-cart-item]:checked").map((el) => el.value);
      local("[data-cart-total]").textContent = money(
        state.routine.products
          .filter((p) => state.selected.includes(p.id))
  ...
  function inputHandler(e) {
    ...
    if (e.target.matches("#sx-custom")) {
      state.context[questions[state.question].key] = e.target.value;
      all('input[name="answer"]').forEach((el) => (el.checked = false));
    }
  ...
  return {
    ...
    destroy() {
      ...
      element.removeEventListener("click", actionHandler);
      element.removeEventListener("submit", submitHandler);
      element.removeEventListener("change", changeHandler);
      element.innerHTML = "";
      element.classList.remove("sb-experience");
    },
  };
}
