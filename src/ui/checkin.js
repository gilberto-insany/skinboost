...
export function showCheckin() {
  openDialog(
    `<p class="eyebrow">ACOMPANHAMENTO / DEMONSTRAÇÃO</p><h2 id="dialog-title">Como foi cuidar<br>de você hoje?</h2><p>O próximo passo também pode ser simplificar.</p><div class="choices" role="group" aria-label="Sua experiência"><button class="choice" aria-pressed="false">Consegui manter</button><button class="choice" aria-pressed="false">Preciso ajustar</button><button class="choice" aria-pressed="false">Ainda não comecei</button></div><label for="checkin-note">Quer contar algo mais? <span class="checkin-optional">Opcional</span></label><textarea id="checkin-note" placeholder="O que funcionou? O que ficou difícil?" maxlength="600"></textarea><p id="checkin-error" role="alert" hidden>Escolha uma das opções para continuar.</p><button id="save-checkin" class="pill dark">Concluir check-in ${icon("check")}</button><p class="checkin-footnote">Exemplo local. Sem conta, envio de dados ou histórico salvo.</p>`,
  );
  let selected = "";
